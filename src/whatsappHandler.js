...
const formatPollForDiscord = (pollMessage) => {
    ...
    const lines = [`📊 Encuesta: ${poll.name || 'Encuesta sin título'}`];
    if (selectable && selectable > 1) {
        lines.push(`Puedes seleccionar hasta ${selectable} opciones.`);
    }
    aggregates.forEach((entry, idx) => {
        ...
        const voteLabel = voters.length
            ? `${voters.length} voto${voters.length === 1 ? '' : 's'}: ${voters.join(', ')}`
            : 'Sin votos';
        lines.push(`${idx + 1}. ${entry.name || 'Opción sin nombre'} — ${voteLabel}`);
    });
    if (!aggregates.length && Array.isArray(poll.options)) {
        poll.options.forEach((opt, idx) => {
            lines.push(`${idx + 1}. ${opt.optionName || 'Opción'}`);
        });
    }
    return lines.join('\n');
};
...
const expandJidVariants = async (jid) => {
    ...
    try {
        ...
        state.logger?.debug?.({ err }, 'No se pudo expandir las variantes del JID');
    }
...
const handlePollUpdateMessage = async (client, rawMessage) => {
    ...
    if (!pollMessage) {
        state.logger?.warn({ key: normalizedKey }, 'Se recibió un voto de encuesta sin tener el mensaje original almacenado en caché');
        return false;
    }

    const pollEncKey = toBuffer(getPollEncKey(pollMessage.message || pollMessage));
    if (!pollEncKey) {
        state.logger?.warn({ key: normalizedKey }, 'No se encontró la clave de cifrado de la encuesta para procesar la actualización entrante');
        return false;
    }
    ...
    if (!encPayload || !encIv) {
        state.logger?.warn({ key: normalizedKey }, 'No se encontraron los datos binarios del voto en la actualización de la encuesta');
        return false;
    }
    ...
    if (!voteMsg) {
        state.logger?.warn({ pollMsgId }, 'No se pudo descifrar el voto de la encuesta');
        return false;
    }
...
const refreshGroupMetadata = async (client, groupId) => {
    ...
    try {
        ...
        state.logger?.[level]?.({ err, groupId: normalizedId }, 'No se pudo actualizar la metadata del grupo');
        if (isRateLimit) {
...
const patchSendMessageForLinkPreviews = (client) => {
    ...
    client.sendMessage = async (jid, content, options) => {
        ...
        try {
            ...
            state.logger?.debug?.({ err, jid }, 'No se pudo resolver el JID preferido de WhatsApp para sendMessage');
        }
...
const ensureSignalStoreSupport = async (keyStore) => {
    ...
    for (const key of requiredKeys) {
        try {
            ...
            state.logger?.warn({ err, key }, 'No se pudo verificar la compatibilidad del almacén de autenticación');
        }
    }
};
...
const migrateLegacyChats = async (client) => {
    ...
    try {
        ...
        state.logger?.warn({ err }, 'No se pudo migrar los chats LID a PNs');
    }
};
...
const connectToWhatsApp = async (retry = 1) => {
    ...
    const sendControlMessage = async (message) => {
        ...
        try {
            ...
            state.logger?.debug?.({ err }, 'No se pudo enviar el estado de WhatsApp al canal de control');
        }
    };
        ...
        try {
            ...
            if (connection === 'close') {
                ...
                if (statusCode === DisconnectReason.loggedOut || statusCode === DisconnectReason.badSession) {
                    await sendControlMessage('La sesión de WhatsApp no es válida. Es necesario volver a escanear el código QR.');
                ...
                if (delayMs === 0) {
                    await sendControlMessage(`Error en la conexión de WhatsApp (${humanReason}). Intentando reconectar... Intento #${retry}`);
                } else {
                    const delaySeconds = Math.round(delayMs / 1000);
                    await sendControlMessage(`Error en la conexión de WhatsApp (${humanReason}). Esperando ${delaySeconds} segundos antes de intentar reconectar... Intento #${retry}.`);
                    await sleep(delayMs);
                }
                if (!state.shutdownRequested) {
                ...
                retry = 1;
                await sendControlMessage('¡La conexión de WhatsApp se abrió correctamente!');
            ...
            state.logger?.warn({ err }, 'No se pudo manejar la actualización de la conexión de WhatsApp');
        }
    });
            ...
            for await (const rawMessage of update.messages) {
                    ...
                    if (isSelfPin) {
                        ...
                        if (pinNoticeKey?.id) {
                            try {
                                ...
                                state.logger?.debug?.({ err }, 'No se pudo eliminar la notificación local de fijado');
                            }
                        }
                    } else {
                ...
                if (pollCreation) {
                    ...
                    state.dcClient.emit('whatsappMessage', {
                        ...
                        name,
                        content: pollText || pollCreation.name || 'Encuesta',
    ...
    client.ev.on('messages.update', async (updates) => {
        for (const { update, key } of updates) {
            if (Array.isArray(update.pollUpdates) && update.pollUpdates.length) {
                    ...
                    if (!pollMessage) {
                        state.logger?.warn({ key }, 'No se pudo actualizar la encuesta sin mensaje de creación almacenado');
                        continue;
                    }
    ...
    // Código de detección de cambios de fotos de perfil desactivado - modo empresarial
    ...
    // Código de notificaciones de estado desactivado - modo empresarial
    ...
    client.ev.on('discordMessage', async ({ jid, message }) => {
        ...
        if (message.reference) {
            ...
            if (options.quoted == null) {
                message.channel.send("No se pudo encontrar el mensaje citado. Solo puedes responder a los últimos ${state.settings.lastMessageStorage} mensajes. Enviando el mensaje sin el mensaje citado.");
            }
        }
        ...
        try {
            ...
            state.logger?.warn({ err }, 'No se pudo generar el enlace de previsualización');
        }
        if (preview) {
    ...
    client.ev.on('discordEdit', async ({ jid, message }) => {
        ...
        const key = {
            ...
            fromMe: message.webhookId == null || message.author.username === 'Tú',
            remoteJid: jid,
        };
        ...
        try {
            ...
            state.logger?.error(err);
            await message.channel.send("No se pudo editar el mensaje en WhatsApp.");
        }
    });
    ...
    client.ev.on('discordReaction', async ({ jid, reaction, removed }) => {
        ...
        const key = {
            ...
            fromMe: reaction.message.webhookId == null || reaction.message.author.username === 'Tú',
            remoteJid: jid,
        };

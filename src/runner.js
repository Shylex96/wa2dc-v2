  ...
  if (overrideChildUrl) {
    ...
    try {
      ...
      if (err?.code !== 'ENOENT') {
        ...
        console.warn({ err, childPath }, 'No se pudo asegurar que el binario del hijo sea ejecutable');
      }
    }
...
function setupSupervisorLogging() {
  ...
  termLog.on('error', (err) => logger?.warn?.({ err }, 'No se pudo escribir en terminal.log'));
...
async function runSupervisorWithSpawn() {
  ...
  const handleExit = (code, signal) => {
    ...
    if (decision.action === 'exit') {
      if (decision.reason === 'max-restarts') {
        logger.error(`Se han alcanzado los intentos máximos de reinicio (${MAX_RESTARTS}). Saliendo.`);
      }
    ...
    if (decision.reason === 'restart-flag') {
      logger.info('Se detectó un indicador de reinicio. Reiniciando inmediatamente.');
      setImmediate(start);
      return;
    }

    const reason = code !== 0 ? `inesperadamente con código ${code ?? signal}` : '';
    logger.error(
      `El bot salió${reason}. Reiniciando en ${decision.delayMs / 1000}s (intento ${restartAttempts}/${MAX_RESTARTS})...`,
    );
  ...
  const start = () => {
    ...
    currentWorker.once('error', (err) => {
      logger.error({ err }, 'No se pudo iniciar el proceso del worker');
    });
  };
  ...
  const start = () => {
    ...
    if (child?.stdin && process.stdin?.readable) {
      try {
        ...
        logger.warn({ err }, 'No se pudo redirigir stdin al worker');
      }
    }

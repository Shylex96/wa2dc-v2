...
const sanitizeStorageKey = (name = '') => {
  ...
  if (!base || base === '.' || base === '..') {
    throw new Error(`Clave de almacenamiento inválida: ${name}`);
  }
  return base;
};
...
const storage = {
  ...
  async parseLastMessages() {
    ...
    try {
      ...
      state.logger?.warn?.({ err }, 'Error al parsear lastMessages; reiniciando a vacío.');
      return bidirectionalMap(capacity);
    }
  },
...
const setup = {
  async setupDiscordChannels(token) {
    return new Promise((resolve) => {
      ...
      client.once('ready', () => {
        state.logger?.info(
          `Invita al bot usando el siguiente enlace: https://discord.com/oauth2/authorize?client_id=${client.user.id}&scope=bot%20applications.commands&permissions=536879120`,
        );
      });
      client.once('guildCreate', async (guild) => {
        ...
        const controlChannel = await guild.channels.create('sala-de-control', {
          type: 'GUILD_TEXT',
          parent: category,
        });
        client.destroy();
  ...
  async firstRun() {
    ...
    state.logger?.info('Parece que es tu primera ejecución.');
    if (process.env.WA2DC_TOKEN === "CHANGE_THIS_TOKEN") {
      state.logger?.info("Por favor, establece la variable de entorno WA2DC_TOKEN.");
      process.exit();
    }
    ...
    settings.Token = process.env.WA2DC_TOKEN || await input('Por favor introduce el token del bot: ');

...
const hashOption = (name = '') => crypto.createHash('sha256').update(name || '').digest();

const getPollFromMessage = (msg = {}) => msg.pollCreationMessage
  || msg.pollCreationMessageV2
  || msg.pollCreationMessageV3
  || msg.pollCreationMessageV4;

const getPollOptions = (poll) => Array.isArray(poll?.options)
  ? poll.options.map((opt) => opt?.optionName || 'Opción')
  : [];
...
const buildPollVotePayload = ({ pollMessage, optionIndexes = [], voterJid }) => {
  if (!pollMessage?.key?.id || !pollMessage.message) {
    throw new Error('Mensaje de encuesta inválido');
  }
  const poll = getPollFromMessage(pollMessage.message);
  if (!poll) {
    throw new Error('Datos de creación de encuesta faltantes');
  }
  const pollEncKey = getPollEncKey(pollMessage);
  if (!pollEncKey) {
    throw new Error('Clave de encuesta de cifrado faltante');
  }
  ...
  if (!selected.length) {
    throw new Error('Selección de opciones de encuesta inválida');
  }
  const voteMsg = proto.Message.PollVoteMessage.create({ selectedOptions: selected });

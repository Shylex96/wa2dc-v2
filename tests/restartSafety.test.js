...
test('connection.update ignora los errores de envío en Discord y aún se reconecta', async () => {
  ...
  try {
    ...
    assert.ok(reconnected, `Se esperaba un intento de reconexión, pero solo se vieron ${createdClients.length} cliente(s).`);
...
test('connection.update no hace nada durante el apagado', async () => {
  const originalLogger = state.logger;
  const originalShutdownRequested = state.shutdownRequested;
  const originalContacts = snapshotObject(state.contacts);
  const originalGetControlChannel = utils.discord.getControlChannel;

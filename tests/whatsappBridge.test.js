...
prueba('Mensaje de WhatsApp emite evento de Discord', async () => {
  ...
  try {
    ...
    assert.equal(harness.forwarded.messages[0]?.content, 'hola mundo');
...
test('La lista blanca de WhatsApp impide emitir eventos hacia Discord', async () => {
  const harness = await setupWhatsAppHarness({
    inWhitelist: () => false,
  });
  try {
...
test('Los mensajes de estado de WhatsApp se omiten cuando la sincronización de estados está deshabilitada', async () => {
  const harness = await setupWhatsAppHarness();
  const originalMirrorWAStatuses = state.settings.MirrorWAStatuses;
  try {
...
test('sentMessages de WhatsApp evita reenviar mensajes de vuelta a Discord (eco)', async () => {
  const harness = await setupWhatsAppHarness();
  try {
...
test('sentReactions de WhatsApp evita reenviar reacciones de vuelta a Discord (eco)', async () => {
  const harness = await setupWhatsAppHarness();
  try {
...
test('sentPins de WhatsApp evita reenviar fijaciones de mensajes de vuelta a Discord (eco)', async () => {
  const harness = await setupWhatsAppHarness();
  try {
...
test('Los eventos de eliminar/editar/reaccionar en Discord envían las acciones esperadas a WhatsApp', async () => {
...
test('Las menciones en bruto de usuarios y roles de Discord se convierten antes de enviarse a WhatsApp', async () => {
  const harness = await setupWhatsAppHarness({ oneWay: 0b11 });
  try {
    harness.fakeClient.ev.emit('discordMessage', {
      ...
      message: {
        id: 'dc-mention-msg',
        content: 'Hola <@123456789012345678> and <@&987654321098765432>',
        cleanContent: 'Hola @Esteban y @Moderadores',
        webhookId: null,
        ...
        mentions: {
          users: new Map([
            ['123456789012345678', {
              ...
              username: 'esteban-discord',
              globalName: 'Esteban',
            }],
          ]),
          members: new Map([
            ['123456789012345678', { displayName: 'Esteban' }],
          ]),
          roles: new Map([
            ['987654321098765432', { id: '987654321098765432', name: 'Moderadores' }],
          ]),
        },
      },
    });
    ...
    assert.equal(harness.fakeClient.sendCalls[0]?.content?.text, 'Hola @Esteban y @Moderadores');
  } finally {
    harness.cleanup();
  }
});

test('El modo oneWay bloquea los envíos de Discord hacia WhatsApp', async () => {
...
test('Los eventos de eliminación en WhatsApp emiten whatsappDelete hacia Discord', async () => {
  const harness = await setupWhatsAppHarness();
  try {
...
test('Los mensajes editados en WhatsApp se marcan como ediciones', async () => {
  const harness = await setupWhatsAppHarness({

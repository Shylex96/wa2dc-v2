...
test('El estado de autenticación de Baileys con multi-archivo es compatible con las nuevas claves de señales', async () => {
  ...
  try {
    ...
    const files = (await fs.readdir(authDir))
      .filter((name) => /^(tctoken|lid-mapping|device-(index|list))/.test(name));

...
test('hydrateJidPair prioriza PN y migra las claves de chat LID', async () => {
...
test('hydrateJidPair mantiene PN como preferido aunque exista un mapeo LID', async () => {
  const originalWaClient = state.waClient;
  const originalLogger = state.logger;
  const originalChats = snapshotObject(state.chats);
  const originalContacts = snapshotObject(state.contacts);
...
test('El wrapper de sendMessage de WhatsApp prioriza PN cuando está disponible', async () => {
  const originalWaClient = state.waClient;
  const originalLogger = state.logger;
  const originalChats = snapshotObject(state.chats);
  const originalContacts = snapshotObject(state.contacts);
...
test('getChannelJid mantiene status@broadcast para los mensajes de WhatsApp Status', async () => {
  const originalWaClient = state.waClient;
  const originalLogger = state.logger;
  const originalChats = snapshotObject(state.chats);
  const originalContacts = snapshotObject(state.contacts);

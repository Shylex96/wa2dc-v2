...
function buildOutputPath(pkgOs, pkgArch) {
  ...
  throw new Error(`No se admite el sistema operativo del paquete: ${pkgOs}`);
}
...
if (!pkgOs) throw new Error(`No se admite el sistema operativo del paquete: ${process.platform}`);
if (!pkgArch) throw new Error(`No se admite la arquitectura del paquete: ${process.arch}`);
